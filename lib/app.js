...
var fetchContainerPathId = function(req, res, next) {
  console.log('fetchContainerPathId');
  console.log(process.env.FS_ID_FILE_ROOT);
  var idFilePath = path.normalize(process.env.FS_ID_FILE_ROOT.replace(
    '%%CONTAINERID%%',
    req.query.container)
  );
  console.log('readFile', idFilePath);
  fs.readFile(idFilePath, 'utf8', function (err, data) {
    if (err) {
      console.log('error sendinc tonaienr mapping file', err, data);
